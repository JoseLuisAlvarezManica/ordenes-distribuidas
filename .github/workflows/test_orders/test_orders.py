...
class TestOrders(unittest.TestCase):
	...
	def setUpClass(cls) -> None:
		...
		orders_db_user, orders_db_password, orders_db_name = cls.resolve_orders_db_credentials()
		...
		notifications_db_user, notifications_db_password, notifications_db_name = cls.resolve_notifications_db_credentials()
	...
	def resolve_notifications_db_credentials(cls) -> tuple[str, str, str]:
		...
		normalized = notifications_url.replace("postgresql+asyncpg://", "postgresql://", 1)
		parsed = urlparse(normalized)
		if not (parsed.username and parsed.password and parsed.path):
	...
	def create_admin_user_via_api(cls) -> None:
		...
		if signup.status_code != 201:
			raise RuntimeError(f"Error creating admin seed user: {signup.status_code} - {signup.text}")
	...
	def login_user(cls) -> str:
		...
		if signup.status_code != 201:
			raise RuntimeError(f"Error creating user: {signup.status_code} - {signup.text}")
		...
		if login.status_code != 200:
			raise RuntimeError(f"Error logging user: {login.status_code} - {login.text}")
	...
	def login_admin(cls) -> str:
		...
		if login.status_code != 200:
			raise RuntimeError(f"Error logging admin: {login.status_code} - {login.text}")
		return login.json()["access_token"]
