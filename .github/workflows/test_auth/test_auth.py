...
class AuthEndpointsTest(unittest.TestCase):
	...
	def request_json(self, method: str, path: str, expected_status: int, token: str | None = None, payload: dict | None = None) -> dict:
	...
	def test_01_admin_login_from_direct_db_seed(self) -> None:
		...
		me_data = self.request_json("GET", "/auth/me", expected_status=200, token=admin_token)
	...
	def test_02_user_auth_endpoints_workflow(self) -> None:
		...
		me_data = self.request_json("GET", "/auth/me", expected_status=200, token=user_token)
		self.assertEqual(me_data.get("email"), self.user_email)
		self.assertEqual(me_data.get("role"), "user")

		refresh_data = self.request_json("POST", "/auth/refresh", expected_status=200, token=user_token)
		...
		logout_data = self.request_json("POST", "/auth/logout", expected_status=200, token=refreshed_token)
