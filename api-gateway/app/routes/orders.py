...
@router.post(
    "/", status_code=status.HTTP_202_ACCEPTED, dependencies=[Depends(bearer_scheme)]
)
@must_be_logged_in
async def create_order(
    ...
    await redis.execute_command(
        "HSET", redis_key, "status", "RECEIVED", "last_update", now
    )
    ...
    try:
        ...
        await writer_client.post(
            "/internal/orders", order_payload, headers={"X-Request-Id": request_id}
        )
        ...
        raise HTTPException(
            ...
            detail={
                "message": "Writer service or database unavailable",
                "order_id": order_id,
            },
        )
...
@router.get(
    "/my_orders", status_code=status.HTTP_200_OK, dependencies=[Depends(bearer_scheme)]
)
@must_be_logged_in
async def get_my_orders(request: Request, writer_client: writer_dependency):
    data = await writer_client.get(
        "/internal/orders/my_orders", headers={"X-Customer": request.state.username}
    )
    return data
